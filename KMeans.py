...
MODULE_NAME= "KMeans" # Declare module name to use as file output name
...
def computeSilhouttee(clusters):
    ...
    for cluster in cluster_arr:
        ...
        for point in points_arr:
            ...
            silhouette_i = (b_i - a_i) / np.max((a_i, b_i))
    ...
    return np.average(silhouette_arr)
...
if __name__ == "__main__":
    ...
    clusters = [ kMeans(dataset, k, MAX_ITER) for k in range(1, MAX_CLUSTER_NUM+1) ]
    # Plot Barchart between the number of clusters and Silhouette Coefficient Score
    plot_silhouttee(clusters)

    # clusters = kMeans(dataset, 3, 10)
    

    print("\n ***** KMeans Clustering has been completed! *****")
